...
function MatchingSleeves() {
  ...
  const getBoardgameDescription = (boardgamegeekId1) => {
    axios.get("https://boardgamegeek.com/xmlapi2/thing?id="+boardgamegeekId1).then((response) => {
      boardgameJSON = xml2json(response.data)
      let clearStringDescription = unEntity(boardgameJSON.items.item.description);
  ...
  for (var i = 1; i <= count; i++) {
    sleeves.push(
      ...
      <Item>
        ...
        <Button
          variant="contained"
          href={foundBoardgame.amazonlink}
        >
  ...
  function unEntity(boardgameDescriptionString){
    return boardgameDescriptionString.replace(/&amp;/g, "&").replace(/&#10;/g, " ");
 }
  ...
  return (
    <div style={{ height: 700, width: '100%' }}>
      <Box sx={{ flexGrow: 1 }}>
        ...
        <br/>
        <Typography gutterBottom={true} variant="overline">Description from boardgamegeek.com</Typography>
        <br/>
